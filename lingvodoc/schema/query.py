...
class CreateAdverbData(graphene.Mutation):
    ...
    def sort_instances(instance_list):
        ...
        for lex, report in adverb_list.items():
            ...
            adverb_list[lex]['entropy'] = entropy([report[case] for case in adverb.cases if report[case] > 0])
            ...
            for p in i['paragraphs']:
                for s in p['sentences']:
                    ...
                    if tuple_s in valency_sentence_dict:
                        valency_sentence_data = valency_sentence_dict[tuple_s]
                        valency_sentence_data.data['instances_adv'] = instance_list
                        sentence_data = valency_sentence_data.data
                        flag_modified(valency_sentence_data, 'data')
                    else:
                        sentence_data = {
                            'tokens': s,
                            'instances': [],
                            'instances_adv': instance_list}
